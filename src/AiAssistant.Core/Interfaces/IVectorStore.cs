...
{
    Task StoreVectorAsync(string id, float[] vector, Dictionary<string, string> metadata, CancellationToken cancellationToken = default);
    Task<List<(float Score, Dictionary<string, string> Metadata)>> SearchSimilarAsync(float[] queryVector, int limit = 5, float scoreThreshold = 0.7f, CancellationToken cancellationToken = default);
